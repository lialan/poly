...
BTCPrediction = CryptoPrediction
BTC15mPrediction = CryptoPrediction
BTC1hPrediction = CryptoPrediction


def get_current_slot_timestamp_15m() -> int:
    """Get the Unix timestamp for the current 15-minute slot."""
    return get_current_slot_timestamp(MarketHorizon.M15)


def timestamp_to_slug_15m(timestamp: int) -> str:
    """Convert Unix timestamp to BTC 15m market slug."""
    return timestamp_to_slug(Asset.BTC, MarketHorizon.M15, timestamp)


def slug_to_timestamp_15m(slug: str) -> Optional[int]:
    """Extract Unix timestamp from BTC 15m market slug."""
    return slug_to_timestamp(slug)


async def fetch_current_15m_prediction() -> Optional[CryptoPrediction]:
    """Fetch the current 15-minute BTC prediction market."""
    return await fetch_current_prediction(Asset.BTC, MarketHorizon.M15)


async def fetch_current_1h_prediction() -> Optional[CryptoPrediction]:
    """Fetch the current 1-hour BTC prediction market."""
    return await fetch_current_prediction(Asset.BTC, MarketHorizon.H1)


async def fetch_btc_15m_prediction(timestamp: int) -> Optional[CryptoPrediction]:
    """Fetch a specific 15-minute BTC prediction market."""
    slug = timestamp_to_slug(Asset.BTC, MarketHorizon.M15, timestamp)
    return await _fetch_prediction_by_slug(slug, Asset.BTC, MarketHorizon.M15)


# Additional backward compatibility
INTERVAL_SECONDS = MarketHorizon.M15.value
get_current_slot_timestamp_btc = get_current_slot_timestamp_15m
